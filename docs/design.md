...
**バージョン:** 1.8.0　　**作成日:** 2026年3月

## 更新履歴

| バージョン | 内容 |
|---|---|
| v1.8.0 | レビュー指摘反映（in_progress タスクのバトル計算除外を明記・DELETE /v1/tasks/:id の処理フロー追加・リフレッシュトークンローテーション方針を注記・GET /v1/tasks の date パラメータ JST 基準を明記・少人数リーグの昇格0人問題を注記・主要テーブル参照ポインタ追加） |
...
> **リフレッシュトークンのローテーション（MVP省略・v1.1以降で対応）：** セキュリティのベストプラクティスとしては、リフレッシュトークンを使用するたびに古いトークンを失効させ新しいトークンを発行する「ローテーション」が推奨される。MVPでは実装コスト削減のため省略し、リフレッシュトークンは30日間の有効期限まで使い回す設計とする。v1.1以降で対応予定。
...
| ステップ | 処理 |
|---|---|
| 1 | `DELETE /v1/tasks/:id` をリクエスト |
| 2 | 指定した `:id` のタスクが他ユーザーのものである場合は `403 FORBIDDEN` を返す |
| 3 | 存在しないタスクIDの場合は `404 NOT_FOUND` を返す |
| 4 | タスクの `status` を確認する |
| 5 | `in_progress`（`end_time IS NULL`）の場合：EXP・ステータスはまだ反映されていないため、`tasks` テーブルからレコードを削除するのみ |
| 6 | `completed`（`end_time IS NOT NULL`）の場合：`tasks` テーブルからレコードを削除する。**付与済みのEXP・ステータスはロールバックしない（不可逆）** |
| 7 | `204 No Content` を返す |

> **EXP不可逆の設計根拠：** completed タスクのEXPをロールバックすると `avatars` テーブルとのトランザクション管理が増え、実装複雑度が上がる。またゲームバランス上「一度得たステータスは消えない」方が自然なため不可逆とする。フロントエンドでは completed タスクの削除前に警告を表示することを推奨する。
...
### 6.6 デイリーバトルフロー（冪等性保証）

> **タイムゾーン基準：** バトルスケジュール（00:00・06:00）およびアクティブユーザー判定の「3日以内」はすべて **JST（UTC+9）** を基準とする。

> **in_progress タスクのバトル計算除外：** バトル計算時（00:00）にステータスを参照する際は、`end_time IS NOT NULL` の完了済みタスクのみを集計したステータスを使用する。進行中タスク（`end_time IS NULL`）のEXPはまだ `avatars` テーブルに反映されていないため、自動的に除外される。深夜0時をまたいで作業中のユーザーは当日のバトル結果に影響しない点に注意。
...
> **少人数リーグの昇格0人問題：** リーグ人数が4人以下の場合、`floor(人数 × 0.2) = 0` となり昇格者が出ない。MVPの初期フェーズでは意図的に許容する。v1.1以降で最低昇格人数（例：1人）の設定を検討する。
...
> **主要テーブルについて：** `users`・`avatars`・`tasks`・`league_memberships`・`battles` など主要テーブルのDDLは `apps/api/src/models/` 配下のマイグレーションファイルで管理する。本セクションでは設計上の注意点が必要なテーブルのみ定義を記載する。
