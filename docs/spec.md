...
> **v1.1予定：** 直前タスクによるステータス急増がバトルに有利に働きすぎる問題（例：バトル直前に集中してタスクをこなすことで当日のみ戦闘力が高くなる）については、過去3日平均ステータスの採用等で対処する予定。
...
> **少人数リーグの昇格0人問題：** リーグ人数が4人以下の場合、`floor(人数 × 0.2) = 0` となり昇格者が出ない。MVPの初期フェーズ（ユーザー数が少ない段階）では意図的に許容する。ユーザーが増加した時点で自然に解消されるため、v1.1以降で最低昇格人数（例：1人）の設定を検討する。
...
| 00:00 | 5 | 各ペアの `base_power = INT×1.1 + STR×1.0 + FOC×1.2 + SPI×1.0` を計算 |
| 00:00 | 6 | `random_range = floor(base_power × 0.1)` のランダム幅を加算した power で1戦ごとに勝敗を決定 |
| 00:00 | 7 | 両者の power が完全に同値の場合はランダムで勝者を決定 |
| 00:00 | 8 | 5回バトルを実施し、3勝以上した方をペアの勝者とする |
| 00:00 | 9 | **トランザクション内で** 全ペアを `battles` テーブルに `is_published = false` で保存し、同一トランザクション内で `battle_executions` に `battle_date = today` を INSERT（非公開） |
| 06:00 | 10 | `is_published = true` に更新 |
| 06:00 | 11 | `league_memberships` の `wins` / `losses` / `match_count` を更新 |
| 06:00 | 12 | 昇格処理（Sリーグは対象外） |
| 06:00 | 13 | 降格処理（Cリーグは対象外） |
...
> **リフレッシュトークンのローテーション（MVP省略・v1.1以降で対応）：** セキュリティのベストプラクティスとしては、リフレッシュトークンを使用するたびに古いトークンを失効させ新しいトークンを発行する「ローテーション」が推奨される。MVPでは実装コスト削減のため省略し、リフレッシュトークンは30日間の有効期限まで使い回す設計とする。漏洩リスクが許容できないフェーズになった時点でv1.1以降に導入する。
...
// Response 200
{
  ...
  "match_count": 16,
  "ranking": [
    { "rank": 1, "user_id": "uuid", "username": "hanako", "wins": 15, "losses": 1, "match_count": 16, "power": 320 },
    { "rank": 2, "user_id": "uuid", "username": "jiro", "wins": 13, "losses": 3, "match_count": 16, "power": 295 }
  ]
}
```
...
| ステータス成長 | EXP計算・タイプ補正込みのステータス更新（トランザクション保証） |
| 自動バトル | デイリーバトルの実行・結果保存（00:00実行・06:00公開・冪等性保証） |
